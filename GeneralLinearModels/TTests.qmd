...
In all general linear models you are trying to compare how much of the variance is explained by a model compared to what's not being explained by a model. However, there are a few outcomes you can be working towards calculating:

To capture how much more is explained by a model than is not explained we calculate $F$

$$
F =  \frac{SS_{explained}/df_{explained}}{SS_{unexplained}/df_{unexplained}}
$$

For comparisons between two conditions we can use T-tests to calculate $t$ (which is the square root of F)

$$
T = \frac{var_{explained}}{var_{unexplained}} 
$$
...
It isn't conventional to calculate $R^2$ for one-sample t-tests, so feel free to skip the next part

::: callout-note
## Variance explained by a one-sample t-test

To summarise what proportion of the variance has been explained, we could calculate the $R^2$ value:

$$
R^2 = \frac{SS_{explained}}{SS_{Total}} = \frac{20473.3}{41025.16} = .499
$$

As this isn't conventional there isn't a function that can be used to double check this
:::
...
[The following explains how to calculate F and $R^2$ values if analysing your data as a model. This isn't necessary if you just want to complete a t-test, but does help clarify how t-tests overlap with ANOVAs and regressions. ]{style="color:red"}
