...
[EXPLANATION HERE]{style="color:red"}

# Using distributions to calculate p-values

[(explained below) to compare to F distributions. You might remember that when discussing [normal distribution](../distributions/normal.html) we could calculate what proportion of the population would have a specific z-score or higher. We use a similar logic frequently in statistics in which we calculate a statistic value that we can compare to a distribution to calculate the likelihood of getting that statistic or a more extreme one.]{style="color:red"}
