...
## OJS
```{ojs}
normal_xlsx = FileAttachment("normal.xlsx").xlsx()
normal_excel = normal_xlsx.sheet(0, { headers: true })
Inputs.table(normal_excel)
```

```{ojs}
Plot.plot({
  marks: [
    Plot.lineY(normal_excel, {x: "height", y: "frequency"}),
    Plot.ruleY([0]),
    Plot.ruleX([150], {stroke: "black"})
  ]
})
```
:::
...
```{ojs}
normal_z_xlsx = FileAttachment("normal_z_scores.xlsx").xlsx()
normal__z_excel = normal_z_xlsx.sheet(0, { headers: true })
Inputs.table(normal__z_excel)
```
...
Plot.plot({
  y: {domain: [0,0.05]},
  marks: [
    Plot.lineY(normal__z_excel, {x: "z-score", y: "Frequency"}),
    Plot.ruleY([0]),
...
```{ojs}
normal_z_0_plus_xlsx = FileAttachment("normal_z_scores_0_plus.xlsx").xlsx()
normal_z_0_plus_excel = normal_z_0_plus_xlsx.sheet(0, { headers: true })
Inputs.table(normal_z_0_plus_excel)
```
...
Plot.plot({
  ...
  marks: [
    Plot.lineY(normal_z_0_plus_excel, {x: "z-score", y: "frequency", stroke: "boundary"}),
    Plot.ruleY([0]),
    Plot.ruleX([0], {stroke: "black"})
  ]
})
...
```{ojs}
normal_z_perc_xlsx = FileAttachment("normal_z_scores_percentages.xlsx").xlsx()
normal_z_perc_excel = normal_z_perc_xlsx.sheet(0, { headers: true })
Inputs.table(normal_z_perc_excel)
```
...
Plot.plot({
  ...
  marks: [
    Plot.lineY(normal_z_perc_excel, {x: "z-score", y: "frequency", stroke: "boundary"}),
    Plot.ruleY([0]),
    Plot.ruleX([0], {stroke: "black"})
  ]
})
...
<div style="color:#068">If you want to practice with different numbers in these questions then please reload the page.</div>