...
For those of you interested in how we get to Šidák's formula, it's a re-organisation of the FWER formula above to focus on calculating Alpha rather than FWER:
...
Good news - no false positives 😺. However, the $\alpha$ threshold is similarly sharp as a **Bonferroni** correction:
...
-   <span style="color:brown">Brown</span>: "Only the first test is a false-positive", p = `r bonferroni_holm_comb$likelihood[4]`, i.e. takes `r bonferroni_comb$likelihood[4]` of the total area of the above figure
-   <span style="color:purple">Purple</span>: "Only the second test is a false-positive", p = `r bonferroni_holm_comb$likelihood[4]`, i.e. takes `r bonferroni_comb$likelihood[4]` of the total area of the above figure
-   <span style="color:grey">Grey</span>: "Neither test is a false-positive", p = `r bonferroni_holm_comb$likelihood[4]`, i.e. takes `r bonferroni_comb$likelihood[4]` of the total area of the above figure
:::
